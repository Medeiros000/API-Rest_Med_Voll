$(function () {
  ...
  var searchTerm = $('#idDaCaixaDeTexto').val(); // Substitua 'idDaCaixaDeTexto' pelo ID real da sua caixa de texto
...
function doSearch(searchTerm) {
  ...
  if (searchTerm == null || searchTerm == "") {
    ...
    apiUrl = `https://voll-med-medeiros000s-projects.vercel.app/html/${currentPage}`;
  } else {
    // Se não estiver vazio, faça uma chamada à API para obter os resultados filtrados
    apiUrl = `https://voll-med-medeiros000s-projects.vercel.app/html/${currentPage}/${searchTerm}`;
  }
  // console.log(apiUrl);
  ...
  $.getJSON(apiUrl, function (data) {
    ...
    if (currentPage === "consultas") {
      ...
      $.each(resultadosPorData, function (dataList, resultados) {
        ...
        divData.html(
          `<p style="color: #339CFF; font-size: 1.125rem; font-weight: 600;" class="d-inline mx-auto">${formatarData(dataList)}</p>` +
