...
// Função para carregar a configuração do servidor
async function loadConfig() {
	try {
		...
		// Fallback para localhost se não conseguir carregar
		appConfig = { API_BASE_URL: "https://vollmed.vercel.app/" };
	}
}
...
$(function () {
	...
	// Carregar configuração antes de fazer pesquisas
	loadConfig().then(() => {
		doSearch();
...
function doSearch(searchTerm) {
	...
	// Usar a configuração carregada ou fallback
	const apiBaseUrl = appConfig ? appConfig.API_BASE_URL : "http://localhost:8080";
	let apiUrl = "";
